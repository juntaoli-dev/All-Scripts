...
var OVERWRITE_EXISTING = true;   // true → replace old links in Vendor Links
  ...
  for (var i = 0; i < data.length; i++) {
    ...
    try {
      if (live) {
        ...
        var vendorFolder = getOrCreateSubFolder_(marketFolder, vendor);
        ...
        //       var dCopy = dFile.makeCopy(safeName_(vendor + ' - ' + market + ' - Deck'), vendorFolder);
...
notes.push('Deck copy disabled'); // optional: uncomment if you want it logged
...
function upsertVendorLinks_(map, live){
  ...
  Object.keys(map).forEach(function(key){
    var r=map[key];
    var rowVals=[[r.market||'', r.vendor||'',r.vendorFolderLink||'', r.deck||'', r.plan||'', r.updatedAt||'', r.notes||'']];
...
// NEW: Helper to fetch Campaign Name from "Inputs" tab
function getCampaignNameFromInputs_() {
  var ss = SpreadsheetApp.getActive();
  var sheet = ss.getSheetByName(INPUTS_TAB_NAME);
  if (!sheet) return null;
  var val = sheet.getRange(CAMPAIGN_NAME_CELL).getValue();
  return val ? String(val).trim() : null;
}
